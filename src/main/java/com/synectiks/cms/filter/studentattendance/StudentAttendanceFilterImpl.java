...
public class StudentAttendanceFilterImpl  {
	...
	private String selectQry = "select distinct st.id, st.student_name, " + 
			"(select a.attendance_status from student_attendance a where a.student_id = st.id and a.attendance_date = ?) as current_date_status, " + 
			...
			"inner join department dt on st.id = dt.student_id " + 
	...
	public List<DailyAttendanceVo> getStudenceAttendance(StudentAttendanceFilterInput filter) {
        ...
        if(filter != null) {
        	int i=0;
            query.setParameter(++i, Integer.parseInt(filter.getAttendanceDate()));
            query.setParameter(++i, Integer.parseInt(filter.getAttendanceDate()));
            query.setParameter(++i, Integer.parseInt(filter.getAttendanceDate()));
            query.setParameter(++i, Integer.parseInt(filter.getAttendanceDate()));
            ...
            if(filter.getStudentName() != null) {
            	query.setParameter(++i, Integer.parseInt(filter.getStudentName()));
            }
        }
