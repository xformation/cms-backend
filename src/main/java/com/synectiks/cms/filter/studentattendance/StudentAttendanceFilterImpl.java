...
//        "where st.branch_id = ? and dt.id = ? and st.batch_id = ? and st.section_id = ? and sb.id = ? ";
    ...
    public List<DailyAttendanceVo> getStudenceAttendance(StudentAttendanceFilterInput filter) {
    	String selectQry = "select st.id, st.student_name, sa.attendance_status as current_date_status, " + 
    			"(select a.attendance_status as prev1_day_status from student_attendance a where a.lecture_id = (select b.id from lecture b where b.attendancemaster_id = (select c.attendancemaster_id from lecture c where c.id = ?) and b.lec_date = date(lc.lec_date) - 1)),  " + 
    			"(select a.attendance_status as prev2_day_status from student_attendance a where a.lecture_id = (select b.id from lecture b where b.attendancemaster_id = (select c.attendancemaster_id from lecture c where c.id = ?) and b.lec_date = date(lc.lec_date) - 2)),  " + 
    			"(select a.attendance_status as prev3_day_status from student_attendance a where a.lecture_id = (select b.id from lecture b where b.attendancemaster_id = (select c.attendancemaster_id from lecture c where c.id = ?) and b.lec_date = date(lc.lec_date) - 3)),  " +
    			"sa.comments "+
